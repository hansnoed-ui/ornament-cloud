...
export function validateDataset(options = {}) {
  ...
  for (const item of constellations) {
    ...
    if (!artistIds.has(item.artistId)) issues.push({ level: "error", code: "UNKNOWN_ARTIST", message: `Unknown artistId in #${item.editorialNumber}: ${item.artistId}` });
    if (!theoristIds.has(item.theoristId)) issues.push({ level: "error", code: "UNKNOWN_THEORIST", message: `Unknown theoristId in #${item.editorialNumber}: ${item.theoristId}` });
...
export function assertDatasetValid(options = {}) {
